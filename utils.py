...
class Sketchparser:
    ...
    """
    def __init__(self):
        x=np.linspace(0, 2**BITS-1, 2**BITS)
        y=np.linspace(0, 2**BITS-1, 2**BITS)
        xx,yy=np.meshgrid(x,y)
        self.vertices = (np.array((xx.ravel(), yy.ravel())).T).astype(int)
    ...
    def draw_circle(self, v_idx, linewidth=2, color="red"):
        ...
        p1 = dequantize_verts(self.vertices[v_idx[0]], n_bits=BITS, 
                                  min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        p2 = dequantize_verts(self.vertices[v_idx[1]], n_bits=BITS, 
                                  min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        p3 = dequantize_verts(self.vertices[v_idx[2]], n_bits=BITS, 
                                  min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        p4 = dequantize_verts(self.vertices[v_idx[3]], n_bits=BITS, 
                                  min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
    ...
    def draw_line(self, v_idx, next_v_idx, linewidth=2, color="black"):
        ...
        start_v = dequantize_verts(self.vertices[v_idx[0]], n_bits=BITS, 
                                 min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        end_v = dequantize_verts(self.vertices[next_v_idx[0]], n_bits=BITS, 
                                 min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
    ...
    def draw_arc(self, v_idx, next_v_idx, linewidth=2, color="green"):
        ...
        start_v = dequantize_verts(self.vertices[v_idx[0]], n_bits=BITS, 
            min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        mid_v = dequantize_verts(self.vertices[v_idx[1]], n_bits=BITS, 
            min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        end_v = dequantize_verts(self.vertices[next_v_idx[0]], n_bits=BITS, 
            min_range=-SKETCH_R, max_range=SKETCH_R, add_noise=False)
        start_v = self.rescale(start_v)
    ...
    def perform(self, pixels): 
        ...
        clean_pixels = pixels[:np.where(pixels==0)[0][0]]
